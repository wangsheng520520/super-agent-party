...
const translations = {
    'zh-CN': {
        ...
        'addemdNotice': '添加供应商后，请返回此页面并选择【词嵌入模型】以继续。词嵌入模型名称中一般包含“embedding”、“ebd”、“bge”等关键词。例如：bge-large-zh等',
      ...
      'bodyPlaceholder': `请输入请求体，使用JSON Schema格式。例如：
{
    ...
    "required": ["name", "age"]
        ...
        'proxyNotice': '请输入代理地址，例如：http://127.0.0.1:7890 或 http://127.0.0.1:10809',
        ...
        'browserNotice1': '1. 点击上方的github链接，下载chrome-mcp-server.zip文件',
        'browserNotice2': '2. 访问`chrome://extensions/`，打开浏览器的开发者模式，加载未打包的扩展程序，上传chrome-mcp-server.zip文件，启用插件',
        'browserNotice3': '3. 在新扩展页面点击连接，将页面中的URL复制到基础URL中，默认为`http://127.0.0.1:12306/mcp`可不替换。',
...
key3`,
        ...
        'customTTS': '自定义TTS',
        'customTTSserver': '自定义TTS服务器地址',
        'customTTSServerPlaceholder': '请输入自定义TTS服务器地址(可以输入多个，一行一个)',
        ...
        'TTSInterface': '语音合成界面',
        'SeparatorNotice1': '如果想要调整语音合成配置，点击跳转',
        ...
        'autoBehaviorToolNotice': '自主行为工具可以让模型能自己添加自主行为，可以用于实现：”三分钟后提醒我/下午三点打开窗帘等“任务。\n行为类型可以分为定时行为（可以理解为定了一个闹钟）、闲置行为（没有输入且没有输出后一段时间会触发）、周期行为（每隔一段时间触发一次，如果重复次数为1，就是延时一段时间自动触发一次）',
        ...
        "defaultView": "默认视图",
        "defaultViewDescription": "显示默认的助手回复内容",
        ...
        'sherpa-onnx-sense-voice': 'Sense Voice(需本地下载)',
    ...
    'en-US': {
        ...
        'modelId': 'Model ID (Can be filled in manually)',
        ...
        'addemdNotice': 'After adding a vendor, please return to this page and select [word embedding model] to continue. Word embedding model names generally contain keywords such as "embedding", "ebd", "bge", etc.For example: bge-large-zh, etc',
      ...
      'bodyPlaceholder': `Enter the body in JSON Schema format. For example:
{
    ...
    "required": ["name", "age"]
        ...
        'manual': 'Manual',
        'wakeWord': 'Wake Word',
        'endWordMode': 'End Word: Available when both wake word and end word modes are enabled. In this mode, the wake state will not automatically sleep; messages containing the end word will trigger sleep state.',  
        ...
        'proxyNotice': 'Please enter the proxy address, such as http://127.0.0.1:7890 or http://127.0.0.1:10809',
        ...
        'browserNotice1': '1. Click the GitHub link above to download the chrome-mcp-server.zip file',
        'browserNotice2': '2. Visit `chrome://extensions/`, enable Developer Mode in your browser, select "Load unpacked" to upload the chrome-mcp-server.zip file, and enable the extension',
        'browserNotice3': '3. Click "Connect" on the new extension page, then copy the URL from the page into the Base URL field. The default is `http://127.0.0.1:12306/mcp`, which does not need to be replaced if unchanged.',
...
Key3`,
        ...
        'SeparatorNotice1':'If you want to adjust the Text To Speech configuration, click Jump',
        ...
        'autoBehaviorToolNotice': "The Autonomous Behavior Tool allows the model to autonomously add behaviors, which can be used to implement tasks such as: `Remind me in three minutes / Open the curtains at three o'clock in the afternoon, etc.`\nBehavior types can be divided into timed behavior (which can be understood as setting an alarm), idle behavior (which triggers after a certain period of no input and no output), and periodic behavior (which triggers at regular intervals; if the number of repetitions is 1, it means it automatically triggers once after a delay).",
        ...
        "defaultViewDescription": "Show the default assistant response content",
        ...
        'sherpa-onnx-sense-voice': 'Sense Voice (need locally downloaded)',
